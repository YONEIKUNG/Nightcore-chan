...
class MusicPlayer(EventEmitter):
    ...
    def resume(self):
        ...
        raise ValueError('ไม่สามารถเล่นเพลงต่อจากสถานะ %s ได้ค่ะ' % self.state)
    ...
    def pause(self):
        ...
        raise ValueError('ไม่สามารถหยุดเพลงจากสถานะ %s ได้ค่ะ' % self.state)
    ...
    def _playback_finished(self):
        ...
        if not self.bot.config.save_videos and entry:
            if any([entry.filename == e.filename for e in self.playlist.entries]):
                print("[Config:SaveVideos] ข้ามการลบเพลงค่ะ ค้นพบเพลงในคิว")
